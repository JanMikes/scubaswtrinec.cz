...
if [ ! -e "packagist" ]
then
    mkdir -p "packagist" &> /dev/null && echo "$(tput setaf 2)Packagist directory created successfully.$(tput sgr0)" || echo "$(tput setaf 1)Failed creating packagist directory!$(tput sgr0)"
fi

# Copying system files into packagist directory
cp libs/.htaccess packagist/.htaccess &> /dev/null || echo "$(tput setaf 1)Failed creating packagist/.htaccess!$(tput sgr0)"
