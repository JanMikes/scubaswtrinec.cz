...
chmod 755 www/webtemp &> /dev/null || echo "$(tput setaf 1)Failed setting write rights to www/webtemp!$(tput sgr0)"

# Clearing temp folder and/or creating it
rm -rf temp &> /dev/null && mkdir temp &> /dev/null && echo "$(tput setaf 2)Temp directory created successfully.$(tput sgr0)" || echo "$(tput setaf 1)Failed creating temp directory with appropriate access!$(tput sgr0)"
chmod 755 temp &> /dev/null || echo "$(tput setaf 1)Failed setting write rights to temp!$(tput sgr0)"
...
if [ ! -d log ]
then
    mkdir log && echo "$(tput setaf 2)Log directory created successfully.$(tput sgr0)" || echo "$(tput setaf 1)Failed creating log directory!$(tput sgr0)"
    chmod 755 log &> /dev/null || echo "$(tput setaf 1)Failed setting write rights to log!$(tput sgr0)"
fi
...
cp libs/web.config log/web.config &> /dev/null || echo "$(tput setaf 1)Failed creating log/web.config!$(tput sgr0)"
...
composer install > /dev/null &> /dev/null && echo "$(tput setaf 2)Composer successfully installed.$(tput sgr0)" || echo "Failed installing composer!"
